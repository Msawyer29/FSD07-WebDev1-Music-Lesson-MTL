...
<x-slot name="header">
    ...
    <div class="px-4 py-2 leading-relaxed border rounded-lg sm:px-6 sm:py-4">
        <strong>{{ $message->user->firstname . ' ' . $message->user->last_name }}</strong>
        <span class="text-xs text-gray-400">{{ $message->created_at->format('Y-m-d H:i:s') }}</span>
        <span class="text-xs text-gray-400">{{ $message->created_at->diffInSeconds() / 60 }} minutes ago</span>
        <p class="text-sm">
            {{ $message->body }}
        </p>
    </div>
